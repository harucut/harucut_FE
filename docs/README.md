...
개념이 없다는 사실 하나에서 조각 프로토콜과 인계 결함이 전부 따라 나온다.
...
| 필수 요청 필드 | 손 대조는 2026-08-28 이 마지막이고 **그 뒤로 목록이 움직였다**(그때 15개 → 오늘 16개) |
...
2. **그래서 base64 조각 프로토콜이 있다.** 회원 결과물은 https URL 이라 주소만 넘기면
   ...
   이유까지 그 주석에 있다).
3. **비회원이 만든 것을 계정으로 옮기려면 localStorage 를 건너야 한다.**
   `apps/web/lib/pendingGuestSave.ts` 가 원본 4장과 만드는 방법을 담아 두고, 로그인 뒤
   `GuestTrialBridge` 가 그걸로 서버 합성을 부른다. 여기가 지금 깨져 있다 — 아래.

### ⚠️ 알려진 결함 — 게스트→회원 인계가 보통 사진에서 실패한다 (2026-09-02 실측)

실제 브라우저에서 쟀다. 추정이 아니다.

| 잰 것 | Chromium | WebKit |
|---|---|---|
| localStorage 용량 | 4.75MB | 4.75MB |
| 슬롯 크기(1700×2400) 사진형 JPEG q=0.92 를 data URL 로 | 1.50MB | 2.31MB |
| 그 4장 합계 | **5.85MB** | **9.02MB** |

보관물에 들어가는 것이 정확히 그 4장이다. 촬영본은 **슬롯 크기를 넘지 않게** 줄여
JPEG 0.92 로 인코딩하고(`apps/web/app/shoot/capture/_hooks/useCaptureFlow.ts`, 불러온 사진은
`apps/web/lib/photoImport.ts`), `storeGuestHandoff`(result/page.tsx)가 그 data URL 넷을
그대로 담는다 — 즉 위 표가 이 코드가 허용하는 최대치다. 합계가 용량을 넘으므로
`setItem` 이 `QuotaExceededError` 를 던진다. 실제 보관물로 재현했다.

터진 뒤에 일어나는 일:

- `setPendingGuestSave` 가 예외를 삼키고 `false` 를 돌려준다. 로그도 리포트도 없다.
- 화면은 문구만 바뀐다 — "…다만 이 결과를 기록으로 옮길 준비는 하지 못했어요"
  (`apps/web/lib/guestTrialStore.ts` 의 `showGuestSavedNotice`). 다운로드는 성공했으므로
  사용자는 잘 끝난 줄 안다.
- 로그인해도 `GuestTrialBridge` 가 꺼낼 보관물이 없다. 인계는 없던 일이 된다.

**이미 이걸 막으려는 장치가 있는데 모자란다.** `useCaptureFlow.ts` 는 촬영본을 슬롯 크기로
줄이면서 주석에 그 이유를 "한도(대개 5MB)를 넘겨 setPendingGuestSave 가 실패한다"라고
적어 뒀다. 슬롯 크기까지 줄여도 넷을 합치면 4.75MB 를 넘는다는 것이 위 실측이다.
`pendingGuestSave.ts` 머리 주석의 "원본 4장은 완성본보다 작다"도 맞는 말이지만,
작아진 것과 한도 안에 드는 것은 다른 말이다.

고치는 방향은 아직 정하지 않았다. 후보 셋 — 보관소를 IndexedDB 로 옮기기(한도가 다르다),
보관 직전에 원본을 한 번 더 줄이기(결과물 화질을 깎는다), 인계를 서버에 맡기기
(= [app-shell-backend-requests.md](./app-shell-backend-requests.md) 의 비회원 합성 경로 요청).
...
| 요금제 정책 | **백엔드** | 위 "요금제" 절 |
| 게스트→회원 인계 | **프론트** | localStorage 한도를 넘겨 실패한다 — 위 "비회원 구조 > 알려진 결함" 절 |
