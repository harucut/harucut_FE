...
`fileSize`·`sourceKeys` 가 그렇게 새어 나갔다. 배열 본문은 항목 기준으로 맞춘다 —
`.filter` 로 걸러 만든 배열 변수와 `T[]` 항목 타입까지 따라간다. 스웨거와 FE 의 본문 모양이
어긋나면 통과도 실패도 시키지 않고 「본문 모양이 다르다」 경고로 남긴다.
본문을 변수로 넘기면 그 변수의 타입까지
...
`lib/photoImport.ts`)은 그대로 둔다. 다만 **이유가 바뀌었다** — 저장소 한도(localStorage
5MB)는 인계를 IndexedDB 로 옮기면서 사라졌다. 지금 남은 이유는 둘이다.

1. **화면이 한 번에 드는 양.** 갤러리 불러오기는 칸 수 × 6장(지금 24장)을 전부 data URL 로
   세션과 DOM 에 담는다(`app/shoot/upload/page.tsx` 의 `PHOTOS_PER_SLOT`). 원본 크기로
   담으면 모바일 웹뷰가 이 흐름만으로 멎는다.
2. **슬롯보다 큰 화소는 어차피 잘린다** — 올리는 시간만 늘린다.

`photoImport` 의 상한은 2026-09-10 에 **긴 변에서 넓이로** 바뀌었다. 긴 변만 재면 슬롯과
방향이 다른 사진의 짧은 변이 먼저 말라붙어 합성이 그것을 늘렸다(16:9 가로 사진이 세로
슬롯에서 1.78배). 그 대신 실제 사진의 출력 화소가 비율만큼 는다(4:3 +33%, 16:9 +78%) —
치른 값과 남는 한계는 `lib/photoImport.ts` 의 `MAX_PIXELS` 주석이 숫자로 들고 있다.
