...
4~8번은 셸과 무관하다(합성 원본 정리, 요금제 한도, 프레임 수정 시각, 셀 누끼 렌더 주체,
합성 실패 코드). 창구가 같아 여기 모아 둔다.
**막는 것은 1~4번이고, 5~8번은 답 대기이거나 「있으면 좋음」이다.**
...
재시도가 원본을 매번 새로 올리던 것은 **이제 아니다.** `composeFourcutOnServer` 가
올리기에 성공한 4장을 그때 쓴 멱등키에 묶어 들고 있다가(`lastUploadedSources`), 같은
멱등키로 다시 오면 굽기도 올리기도 건너뛴다. 결과 화면의 「다시 준비하기」는 `retryNonce`
를 멱등키에서 빼 뒀으므로(`apps/web/app/shoot/result/page.tsx`) 같은 키로 돌아온다 —
**같은 탭에서 버튼을 몇 번 눌러도 원본은 한 벌뿐이다.**

그래서 아래 수치는 "재시도 1회당 4개"가 아니다. 남는 경로만 남았고, 넷이다.

- **올리다 실패한 판**은 캐시되지 않는다(성공한 뒤에만 기억한다). 위의 첫 경로 그대로
  이미 올라간 나머지가 남고, 다음 시도는 4장을 새로 올린다.
- **새로고침·새 탭**이면 캐시가 없다. 모듈 스코프 변수라 문서 수명만큼만 산다.
  게스트 인계 재시도가 정확히 이 경로다 — `GuestTrialBridge` 는 "이 화면을 새로고침하면
  다시 시도해요"로 안내하고, 그 새로고침이 캐시를 지운다. 멱등키는
  보관물에 심어 두고 다시 쓰므로 **결과물은 한 벌**이지만 원본은 4장 더 올라간다.
- **한 벌만 들고 있다.** 다른 멱등키의 합성이 사이에 끼면 그 자리를 덮는다(프레임을
  바꿔 한 번 합성해 보고 되돌아오는 길). 덮인 쪽은 다시 올린다.
- **입력이 진짜 달라지면** `ensureComposeIdempotencyKey` 가 새 키를 만든다 —
  프레임·테마·레이아웃·필터·색·원본 중 하나라도 바뀔 때다. 이때의 새 업로드는 옳다.
  다만 앞선 합성이 실패로 끝났다면 그 4장은 그대로 남는다.

접수 이후(`compose` 400/폴링 시간 초과) 실패로 남는 4장은 캐시와 무관하게 그대로다.
캐시는 **같은 시도를 반복할 때 사본이 불어나는 것**을 막을 뿐, 한 번 올라간 것을
없애지는 못한다.
...
- 같은 멱등키로 다시 오면 이미 올려 둔 key 를 그대로 쓴다(위 `lastUploadedSources`)
  — 회귀 고정은 `apps/web/lib/fourcutCompose.test.ts` 의
  「composeFourcutOnServer — 같은 멱등키면 원본을 다시 올리지 않는다」 셋
...
단, **출력 지문이 달라진 저장에만** 버린다(저장 직전 `buildFrameContentKey(themeJson)` 를
불러올 때 잡아 둔 값과 비교한다) — 이름만 고쳤거나 레이어를 잠그기만 한 저장까지 버리면
같은 그림이 새 멱등키로 다시 접수돼 보관함에 두 벌이 남는다.
...
## 8. 합성이 실패하면 사유 문자열 말고 **에러 코드**도 주세요 (있으면 좋음)

**무엇을**: `ComposeJobResponse` 에 실패 코드 한 필드(`failureCode` 같은 이름). 값은 우리가
문구로 옮길 수 있는 **닫힌 집합**이면 된다 — 원본을 읽지 못함 / 프레임 자산을 읽지 못함 /
그 외 정도로 충분하다. `failureReason` 문자열 자체를 닫힌 열거로 못박고 그 목록을 스웨거에
적어 주는 것도 같은 효과다.

**지금**: 필드가 넷이고(`jobId`·`status`·`mediaId`·`failureReason`) 실패 때 오는 것은
문자열 하나다(2026-09-07 `/v3/api-docs`).

```json
{"jobId":12,"status":"FAILED","failureReason":"원본 이미지를 읽을 수 없습니다"}
```

**왜**: 그 문자열은 화면에 쓸 수 없다. 스웨거가 그 필드 설명에 **"사용자에게 그대로 보여줄
문구는 아니다"** 라고 적어 뒀고, 그래서 프론트는 값을 통째로 버린다
(`lib/fourcutCompose.ts` 의 `describeComposeFailure` — 원문은 개발 빌드 콘솔에만 남긴다).
버리고 나면 갈라 쓸 것이 남지 않는다. 코드가 없으니 **원본을 읽지 못한 실패든, 프레임 자산을
읽지 못한 실패든 전부 같은 한 문장**으로 끝난다 —
"서버가 네컷을 완성하지 못했어요. 다른 프레임으로 시도해 주세요."

코드가 있는 실패는 이미 갈라 준다. 같은 함수가 `GEN-002`·`GEN-003` 이면 "프레임에 넣은
스티커나 글자를 서버가 읽지 못했어요…", `GEN-031`·404 면 없는 프레임으로 안내한다.
못 가르는 것은 **합성 잡이 `FAILED` 로 끝난 경우 하나**뿐이고, 하필 그 자리가 **되돌릴 수 없는
실패**라(다시 눌러도 결과가 같다) 안내가 정확해야 하는 자리다. 지금은 "다른 프레임으로"라고만
말하는데, 원본을 읽지 못한 실패였다면 그 안내가 틀렸다.

**프론트가 할 일 (열리면)**: `describeComposeFailure` 의 `ComposeFailedError` 분기에 코드별
문구를 더한다. 고정 한 문장이 나가는 자리 하나만 바뀐다.

---

## 참고 — 이번 세션에 확인한 것 (2026-09-01)

**1~4번은 오늘도 그대로 막혀 있고, 5~7번은 답 대기다**(8번은 2026-09-07 에 새로 붙였다).
로컬 백엔드
...
| 6. 프레임 수정 시각 | `FrameResponse` 에 `updatedAt`·`version` 이 **없다**(2026-09-02 `/v3/api-docs`). 프론트는 조회한 내용의 지문으로 우회하고(`lib/shootSessionStore.ts` `buildFrameContentKey`), 편집기에서 **출력 지문이 달라진 저장**이면 멱등키도 버린다(`ThemeEditorPage` → `noteRemoteFrameEdited`) | **답 대기 · 급하지 않음** |
...
| 8. 합성 실패 코드 | `ComposeJobResponse` 에 코드 필드가 **없다** — `jobId`·`status`·`mediaId`·`failureReason` 넷뿐(2026-09-07 `/v3/api-docs`). 스웨거가 `failureReason` 을 "사용자에게 그대로 보여줄 문구는 아니다" 라고 적어 둬 프론트는 그것을 버리고 고정 문구를 쓴다 | **새 요청(2026-09-07)** |
