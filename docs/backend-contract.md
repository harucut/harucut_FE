...
| **꾸미기(스티커·드로잉) 저장** | 완성본 위에 얹은 그림이라 "원본 4장"으로 표현할 수 없다. 완성 이미지를 등록하는 API 가 사라졌다(405) |
| **비회원 결과를 로그인 후 기록으로** | 위와 같은 이유. 원본 4장은 로그인 과정에서 페이지가 다시 뜨며 사라진다(세션 스토어는 메모리) |
...
앞의 셋은 백엔드에 수단이 생겨야 풀린다. 뒤의 둘은 프레임 저장 쪽 작업이다.
