...
## 아직 안 쓰는 백엔드 기능 — 약관 동의

`GET /api/terms`(공개 목록) · `POST /api/auth/terms/consents`(동의·철회) ·
`GET /api/auth/terms/consents/me`(내 동의 상태, `AGREED`/`NEEDS_RECONSENT`/`NOT_AGREED`)가
전부 살아 있고, `TermsAgreementHistoryResponse` 는 **"법적 증빙용, 수정·삭제되지 않는다"**
고 적혀 있다.

우리 가입 화면은 약관·개인정보·마케팅 동의를 받아 놓고 **어디에도 보내지 않는다**
(`app/signup/page.tsx`). `POST /api/harucut/register` 에 동의 필드가 없는 것은 맞지만,
동의는 저 전용 API 로 기록하게 되어 있다. 그 API 는 인증이 필요해서 가입 직후가 아니라
**로그인 이후**에 불러야 하고, `NEEDS_RECONSENT` 를 다루는 재동의 화면도 필요하다 —
기능 하나를 새로 만드는 일이라 별도 논의가 필요하다.
