...
| [`app/shoot/upload/page.tsx`](../apps/web/app/shoot/upload/page.tsx) | **화면이 열린 뒤** | `accessMode` + `resolveMembership()` |
...
화면 쪽은 쿠키만 보고 내보내지 않는다. 게스트 쿠키가 남아 있는데 세션은 멀쩡한 회원이
있어서다(이 판정이 붙기 전 배포에서 체험을 눌러 본 사람). `resolveMembership()`으로 물어
**확정된 `guest`만** 내보내고, `member`면 낡은 쿠키를 걷으며, `unknown`이면 내보내지 않는다.
다만 `unknown` 동안 **쓰게 두지도 않는다** — 회원으로 확인될 때까지 사진 고르기를 잠근다.
그러지 않으면 이미 게스트인 사람이 조회 실패를 틈타 갤러리 불러오기를 끝까지 쓴다.
