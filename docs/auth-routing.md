...
2. 보호 경로가 아니면 그대로 통과
3. **행사 QR 진입(경로가 정확히 `/shoot` + 공백 아닌 `event` 쿼리)이면 통과** — 응답에
   게스트 쿠키를 심는다. **인증 쿠키 판정(6)보다 먼저 본다**(아래 절 참고). 다만 둘 중
   하나라도 걸리면 심지 않고 아래로 내려간다 — 이미 게스트 쿠키가 있거나, **살아 있는
   access**(`hasLiveAccessToken`)가 보일 때
...
"로그인"으로 읽습니다. 그래서 3(행사 진입)과 5(게스트 차단)를 둘 다 6보다 앞에 뒀습니다.
6이 3보다 먼저였을 때는 예전에 로그인했던 브라우저에 남은 죽은 쿠키가 3을 가려서, 행사
참가자가 회원으로 읽힌 채 인증 API에서는 401을 받았습니다. 6이 5보다 먼저였을 때는
...
/shoot?...&event=... + 죽은 access 만              -> 통과 (게스트 쿠키를 심는다)  ← 3이 6보다 먼저
/shoot?...&event=... + refresh 쿠키만              -> 통과 (게스트 쿠키를 심는다)  ← 회수됐는지 알 수 없다
...
(`hasLiveAccessToken`). **`refreshToken`은 살아 있음의 근거로 쓰지 않습니다** — 다른 기기
로그인으로 서버가 회수해도 브라우저에는 그대로 남아, 쿠키만으로는 산 것과 구별되지 않기
때문입니다(아래 「행사 QR 진입」 절).
...
못했다.** 행사 QR 분기가 refresh를 회원 근거로 보지 않게 된 뒤로 그 조합은 행사장에서
그대로 만들어진다 — 예전에 로그인해 둔 브라우저로 QR을 찍으면 체험 쿠키가 심기고 refresh
쿠키는 남는다. 화면 쪽 방어도 없다:
...
"대개"가 이 분기를 위 6보다 **앞에** 둔 이유입니다. 예전에 이 브라우저로 로그인했던
사람도 QR을 찍습니다. 6이 먼저면 만료됐거나 모양이 깨진 `accessToken` 쿠키만 남아 있어도
...
- 판정 조건 셋을 **모두** 만족할 때만 심습니다.
  ...
  3. **살아 있는 access(`exp` 기준)가 없을 것**(`hasLiveAccessToken`). 여기서 심는 체험
     쿠키는 7일을 살고 그동안 `accessMode`가 이 쿠키만 보므로, 지금 회원인 것이 확인되는
     사람은 덮지 않습니다.
- **`refreshToken` 쿠키는 더 이상 막지 않습니다.** 예전 판정(`hasRecoverableSession`)은
  refresh가 있기만 해도 심지 않았는데, 서버가 회수한 refresh는 브라우저에 그대로 남아
  살아 있는 것과 구별되지 않습니다(다른 기기 재로그인이면 Redis의 `REFRESH_TOKEN` 키가
  갈립니다 — [`docs/backend-contract.md`](./backend-contract.md) 「토큰」). 그래서 access는
  만료되고 refresh는 회수된 브라우저 — 행사장에 흔한, 예전에 로그인해 둔 그 브라우저 —
  가 체험 쿠키를 못 받고 위 6으로 통과했고, 화면은 회원인데 인증 API는 전부 401이었습니다.
  되살아날지를 쿠키로 점치는 대신 **확인되는 것**(access의 `exp`)만 근거로 씁니다.
- 그래서 **죽은 access 쿠키나 refresh 쿠키만 든 방문자는 이제 게스트가 됩니다.** 예전 문서가
  "쿠키가 하나도 없어야 한다", "의심스러우면 심지 않습니다"라고 적었던 자리입니다.
- 콜백 분기(위)와 방향이 반대인 것은 의도한 것입니다. 거기서는 확인되지 않으면 **지우지**
  않고, 여기서는 확인되지 않으면 **심습니다** — 방문자가 손해를 보는 쪽이 서로 다릅니다.
  둘 다 기준은 하나입니다: **살아 있는 access가 보일 때만 "지금 회원"으로 취급한다.**
...
- **남는 한계**: access만 자연 만료되고 refresh는 멀쩡한 회원이 이 주소를 열면 7일짜리 체험
  쿠키를 받습니다. QR 진입만의 이야기가 아닙니다 — 같은 주소가 촬영 화면의 "프레임 다시
  선택" 목적지이기도 해서
  ([`apps/web/app/shoot/capture/page.tsx`](../apps/web/app/shoot/capture/page.tsx)의
  `backToFrameHref`), 행사 촬영 도중 access가 만료된 회원이 그 버튼을 눌러 이 주소를 다시
  열면 자기 촬영 중간에 게스트로 뒤집힙니다. 새로고침이면 확실하고, 앱 안에서의 이동도
  RSC 요청으로 프록시를 지나가지만 그때 `Set-Cookie`가 실제로 브라우저에 남는지는
  **확인하지 않았습니다.** 되돌리는 길은 있습니다(공개 화면의 촬영 CTA 한 번 —
  `lib/usePublicShootCta.ts`가 서버로 확인하고 쿠키를 걷습니다 — 또는 재로그인).
  반대쪽(행사 참가자가 아예 못 찍는 것)에는 그런 길이 없어서 이쪽을 골랐습니다.
- 회귀 고정: [`apps/web/proxy.test.ts`](../apps/web/proxy.test.ts)의 「proxy 행사 QR 진입」 —
  "쿠키가 하나도 없으면 체험 쿠키를 심어 통과시킨다", "형식이 깨진/만료된/페이로드가 깨진
  access 쿠키만 남아 있어도 체험 쿠키를 심는다", "회수된 refresh 쿠키를 들고 와도 체험
  쿠키를 심는다", "살아 있는 access를 든 방문자는 게스트로 만들지 않는다", "이미 체험
  중이면 쿠키를 다시 심지 않는다", **"행사 진입이 아닌 주소는 로그인으로 보낸다"**(`event`
  쿼리가 없거나 공백뿐일 때, 그리고 `/shoot/upload`·`/shoot/capture` 처럼 `/shoot` 하위일 때),
  **"심는 체험 쿠키의 속성은 클라이언트와 같은 값이다"**, 그리고
  `apps/web/tests/e2e/guards.spec.ts`의 "lets an event QR visitor shoot without signing up".

이 예외를 지우거나 6 뒤로 되돌리면 행사(B2B) 흐름이 통째로 죽습니다.
