...
### 1.4 배경 이미지 — 토대 존재
- 웹에 `apps/web/lib/themeBackground.ts`, `lib/canvas/composeFrame.ts` 존재 → 배경 합성 토대 있음. `ThemeBackground` 타입(`COLOR`/`IMAGE`/`VIDEO`)은 모바일 `harucut-data.ts`에도 이미 정의됨. → §5에서 활용.
...
- **카메라 = 슬롯 실제 풋프린트**: 촬영 화면을 *전체 프레임 렌더 + 현재 칸에 라이브 카메라* 구조로(권장, §3.4 결정 대기). 차선책은 박스 폭을 슬롯의 프레임 내 상대폭(`slot.width/totalWidth`)에 맞춰 축소.
- **기본 프레임 촬영 가이드**: `themeData`가 없어도 현재 슬롯 테두리/번호/보더색 가이드를 항상 표시(빈 카메라 방지).
- **모바일 동일 적용**: 위 좌표 규칙을 그대로 모바일 오버레이(§3.1~3.3)에 적용해 웹/모바일·미리보기/출력 4면이 모두 일치하도록.
...
- **데이터**: `ThemeBackground`(`COLOR`/`IMAGE`/`VIDEO`)가 모바일·웹 타입에 이미 존재. `IMAGE.key`(원격) 또는 로컬 uri로 배경 지정.
...
| B | **모바일 카메라 오버레이** ★ | 4종+사용자 프레임 실기기, 결과물 좌표 일치 | 핵심·중위험(신규) |
...
- **카메라 오버레이 좌표계**: 오버레이 미리보기와 `captureRef` 최종물의 좌표가 어긋나면 "보이는 것과 결과가 다름" → §3.3 검증으로 차단. (최고 리스크)
