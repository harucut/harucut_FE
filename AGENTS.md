...
- 네이티브가 맡는 것은 여덟 가지다: 사진첩 저장 · 공유 시트 · 햅틱 · **알림** · **상태바 색** ·
  **안전영역** · **카메라 권한(안드로이드)** · 하드웨어 뒤로가기. **이 목록의 소유자는
...
- `scripts/`: 검증 스크립트는 `verify_workspace.py`, `check_backend_contract.py`,
  ...
  `camera-probe.html`(실기기에서 열어 카메라를 실측하는 페이지)와
  `gen-notification-icon.mjs`(알림 아이콘 생성기).
...
위의 commit/push 뿐이다 — **브랜치 이름 형태는 검사하지 않는다.** pre-commit 의 에러 문구는
`<type>/<slug>` 브랜치로 옮기라고 안내하지만, 안내일 뿐 강제되지 않는다 — 사람이 지키는 규칙이다.
...
계약 검사는 `pnpm check:contract`(로컬 백엔드 필요). 경로·죽은 프록시·에러코드에
**FE 가 실제로 보내는 본문 ↔ 스웨거 required** 까지 넷(A·B·C·D)을 본다 —
**필수 필드가 빠지면 종료코드 1 이다.** 사용법과 한계의 소유자는
